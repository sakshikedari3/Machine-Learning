...
def pair_plot(datapath) :
    ...
    sns.pairplot(df)
    plt.title("Pair Plot")
    plt.savefig("Artifacts/Breast_cancer/pair_plot.png")
    plt.close()
...
def main() :
    try :
        ...
        # 1)open file and folder
            file = Open_Folder_file(file_path)

            # 2)read dataset
            df = read_csv(dataset)

            # 3)Store data set to 'CANCER.CSV'
            ...
            # 9)Alter the dataset by dropping unnecessary columns
            x, y = alter(df)
